...
Von den Bodenstationen wurden Radiowellen bekannter Frequenz (S-Band, ~2,11 GHz) zum Satelliten gesendet (uplink). Die Frequenz wird mithilfe eines Wasserstoff-Masers erzeugt.
Damit wird eine äußerst präzise und stabile Frequenz von 5 MHz und 10 MHz erzeugt. Im Digital Controlled Oscillator (DCO), werden diese Frequenz verwendet um mit frequenzmultipliern ein Signal mit ~22 MHz zu erzeugten, welches dann mit dem Faktor 96 multipliziert wird um das zu sendende Signal von 22 GHz zu erhalten.\cite{Anderson2002}
...
Angaben dazu welche DSN Antennen das Signal geschickt und welche es empfangen haben.\cite{Levy2008} 	% ursprünglich magnetbänder?

% (wo) kommt das rein:
%% Da die im ODF format gespeicherten Messdaten nur die Zeit t2 (Signal an der Sonde) nicht jedoch die Zeitpunkte t1
%% (Sendezeitpunkt auf der Erde) und t3 (Empfangszeitpunkt auf der Erde) enthalten, müssen dise für die spätere Verwendung
%% in Analysen durch Schritweiße Berechnung der "relativistic light time equations"" aus t2 errechnen.\cite{Levey2009} %FIXME: begriff übersetzen
